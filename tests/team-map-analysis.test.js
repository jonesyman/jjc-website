...
test("Team Map names use print-weighted capacity flow",()=>{const html=fs.readFileSync("admin/index.html","utf8");assert(html.includes("--team-map-name-size:16px")&&html.includes("--team-map-name-size:12.5px")&&html.includes("font-weight:600")&&html.includes("list.children.length > 12")&&html.includes("list.children.length > 24 ? Math.ceil(list.children.length / 2) : 12")&&html.includes("requestAnimationFrame(fitTeamMapNames)"),"capacity-based name flow missing");});
