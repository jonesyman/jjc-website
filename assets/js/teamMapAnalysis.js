...
window.TeamMapAnalysis = (() => {
  ...
  function suggestedAnalysis(distribution) { const obs=generateTeamObservations(distribution); return { OverallTeamPattern:obs.slice(0,2).map(x=>`${x.type}: ${x.label}`).join("; "), MostSignificantStrength:obs.find(x=>/represented|overuse/i.test(x.label))?.text||"Consider exploring where the team experiences its strongest sources of energy.", MostSignificantRisk:obs[0]?.text||"No major automatic risk rose above the configured thresholds.", CompetencySustainabilityObservation:obs.find(x=>/competency/i.test(x.label))?.text||"Review whether frequently requested competency work is sustainable for the people providing it.", LeaderInfluence:distribution.leaderWeightingApplied?"Leader weighting is included for the leader's two Geniuses and two Frustrations; consider how that influence shapes the team's environment.":"Leader weighting is not included in this analysis.", Recommendations:generateConsultantQuestions(distribution).slice(0,2).join(" "), AdditionalConsultantNotes:"" }; }
