(function(){
  ...
  function renderResources(){
    ...
    byId("postSessionResourceChoices").innerHTML=rows.map(row=>`<label class="archive-toggle package-choice"><input type="checkbox" value="${safe(row.ResourceID)}" ${String(row.DefaultSelected).toLowerCase()==="true"?"checked":""}> <span><strong>${safe(row.Title)}</strong><small>${safe(row.Description||row.FileName)}</small></span></label>`).join("")||'<p class="muted small">No active resources are available.</p>';
  }
